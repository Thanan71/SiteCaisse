<template>
  <div class="ventes-page">
    ...
    <TabNav
      v-model="activeTab"
      :tabs="[
        { key: 'journalier', label: 'Journalier', icon: '📆' },
        { key: 'mensuel', label: 'Par mois', icon: '📅' },
        { key: 'toutes', label: 'Toutes les ventes', icon: '📋' },
      ]"
      @change="setActiveTab"
    />
    ...
    <div v-if="activeTab === 'journalier'">
      ...
      <div v-else>
        ...
        <SummaryCard
          ...
          :total-commission="dailySummary.commission_cb"
          :commission-detail="dailySummary.commission_cb > 0 ? `Taux : Permanent ${dailySummary.taux_commission}%` : ''"
          variant="warning"
        />
      </div>
    </div>
    <div v-if="activeTab === 'mensuel'">
      ...
      <DataState
        :loading="rapportsStore.loadingMois && !rapportsStore.rapportMois"
        :empty="!rapportsStore.loadingMois && !rapportsStore.rapportMois"
        empty-icon="📅"
        empty-title="Sélectionnez un mois"
        empty-message="Choisissez un mois dans le sélecteur ci-dessus"
        loading-text="Chargement des ventes du mois..."
      >
        <div class="month-block">
          <h2 class="month-title">{{ formatMonthLabel(selectedMonth) }}</h2>

          <DataState
            :empty="!rapportsStore.rapportMois.groupes.length"
            empty-icon="📭"
            empty-title=""
            empty-message="Aucune vente pour ce mois"
          >
